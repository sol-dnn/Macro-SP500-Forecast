eps_curr = pd.to_numeric(df_factors['wsf_eps_ltm'], errors='coerce')  # trailing EPS (ou LTM)

# Croissances 12m (avec garde-fou sur les EPS ~0)
floor = 1e-6
g_ours = np.log((eps_hat  ).clip(lower=floor)) - np.log((eps_curr).clip(lower=floor))
g_cons = np.log((cons     ).clip(lower=floor)) - np.log((eps_curr).clip(lower=floor))

# Différence de croissance anticipée
df_factors['GROWTH_DIFF'] = g_ours - g_cons        # >0 : on voit + de croissance que le consensus

# Option "yield": ramener au prix pour un scale comparable à FEY
df_factors['GROWTH_DIFF_P'] = df_factors['GROWTH_DIFF'] / price.replace(0, np.nan)
